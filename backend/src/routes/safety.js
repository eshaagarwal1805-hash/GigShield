...
router.post('/sos', protect, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to create SOS alert', error: err.message });
  }
});
...
router.get('/sos/active', protect, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch active SOS', error: err.message });
  }
});
...
router.patch('/sos/:id/resolve', protect, async (req, res) => {
  ...
  { resolved: true, resolvedAt: new Date() },  // ← set it here directly
  { new: true }
);
    if (!alert) {
    ...
    res.status(500).json({ message: 'Failed to resolve SOS', error: err.message });
  }
});
...
router.post('/report', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to submit risk report', error: err.message });
  }
});
...
router.get('/nearby', protect, async (req, res) => {
  try {
    ...
    if (!lng || !lat) return res.status(400).json({ message: 'lng and lat are required' });
...
router.get('/alerts/nearby', protect, async (req, res) => {
  try {
    ...
    }).select('type message location triggeredAt').limit(10);
    res.json(alerts);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
